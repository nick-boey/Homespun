...
public class ClaudeSessionServiceStatusBroadcastTests
{
    ...
    // Per-type AGUI broadcast tests removed as part of a2a-native-messaging Phase 9: the
    // legacy SDK assembly path no longer broadcasts AG-UI events. SessionEventIngestor
    // now emits SessionEventEnvelope payloads from the A2A event stream — the equivalent
    // coverage lives in SessionEventIngestorTests + A2AToAGUITranslatorTests.
}
