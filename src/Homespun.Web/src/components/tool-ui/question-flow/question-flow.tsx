    ...
    >
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} className="bg-muted relative flex-1 overflow-hidden rounded-full">
          <div
            className={cn(
              'bg-primary absolute inset-0 origin-left rounded-full',
  ...
  return (
    ...
    >
      ...
      {mode === 'single' && isSelected && (
        <span className="motion-safe:animate-in motion-safe:fade-in motion-safe:zoom-in-75 size-2 rounded-full bg-current motion-safe:duration-300 motion-safe:ease-out" />
      )}
    </div>
  )
}
        ...
        <div className="flex flex-col">
          {choice.summary.map((item, index) => (
            <Fragment key={index}>
              ...
              <div
                className="motion-safe:animate-in motion-safe:fade-in motion-safe:blur-in-sm motion-safe:slide-in-from-bottom-1 motion-safe:fill-mode-both flex flex-col gap-0.5 text-sm motion-safe:duration-300 motion-safe:ease-out"
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isExiting && [
          'absolute inset-0',
          'motion-safe:animate-out motion-safe:fade-out motion-safe:blur-out-sm motion-safe:fill-mode-forwards motion-safe:duration-250 motion-safe:ease-[var(--cubic-ease-in-out)]',
          exitClass,
        ],
        !isExiting &&
          isTransitioning && [
            'motion-safe:animate-in motion-safe:fade-in motion-safe:blur-in-sm motion-safe:fill-mode-both motion-safe:duration-250 motion-safe:ease-[var(--cubic-ease-in-out)]',
            enterClass,
          ]
      )}
