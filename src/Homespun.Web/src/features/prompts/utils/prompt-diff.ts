...
export type PromptChanges = {
  ...
  updates: ExportedPrompt[]
  deletes: string[] // names of prompts to delete
}
...
export function calculateDiff(current: ExportedPrompt[], edited: ExportedPrompt[]): PromptChanges {
  ...
  const updates: ExportedPrompt[] = []
  const deletes: string[] = [] // names
  ...
  for (const prompt of edited) {
    ...
    if (!currentPrompt) {
      // New prompt (name not in current set)
      creates.push(prompt)
