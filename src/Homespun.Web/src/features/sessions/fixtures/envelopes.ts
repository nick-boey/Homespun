...
/**
 * Sequential tool turns where each tool call lives in its own SDK assistant
 * message (different `parentMessageId` per call). Mirrors the typical Claude
 * Code execution pattern where the model calls one tool, waits for the
 * result, then issues the next tool in a new assistant message.
 *
 * Without coalescing, AUI's `ToolGroup` would render four single-tool groups
 * because each tool lives in its own `AGUIMessage`. With coalescing, all four
 * collapse under one group.
 */
export const sequentialToolMessages: SessionEventEnvelope[] = [
  env(1, 'e1', { type: 'RUN_STARTED', threadId: SESSION_ID, runId: 'r1', timestamp: 0 }),
  // Tool 1 (assistant message a1)
  env(2, 'e2', {
    type: 'TOOL_CALL_START',
    toolCallId: 'tc-bash',
    toolCallName: 'Bash',
  ...
  // Tool 2 (assistant message a2 — separate parentMessageId)
  env(6, 'e6', {
    type: 'TOOL_CALL_START',
    toolCallId: 'tc-read',
    toolCallName: 'Read',
  ...
  // Closing summary text in yet another assistant message
  env(18, 'e18', {
    type: 'TEXT_MESSAGE_START',
    messageId: 'a5',
    role: 'assistant',
  ...
  env(20, 'e20', { type: 'TEXT_MESSAGE_END', messageId: 'a5', timestamp: 19 }),
  env(21, 'e21', { type: 'RUN_FINISHED', threadId: SESSION_ID, runId: 'r1', timestamp: 20 }),
]
