    ...
    IContainerQueryService containerService) : ControllerBase
{
    ...
    public async Task<IActionResult> SendMessage(string id, [FromBody] SendMessageRequest request)
    {
        ...
        try
        {
            // Use the session's existing mode if no mode is specified in the request
            var mode = request.Mode ?? session.Mode;
            await sessionService.SendMessageAsync(id, request.Message, mode);
            return Accepted();
        }
        catch (Exception ex)
        {
...
// All request models are defined in Homespun.Shared.Requests
