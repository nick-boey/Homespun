...
public class PullRequestWorkflowService
{
    ...
    public PullRequestWorkflowService(
    ...
    private async Task<PullRequestStatus> DetermineStatusAsync(string owner, string repo, PullRequest pr)
    {
        ...
        try
        {
            commitStatus = await _githubClient.GetCombinedCommitStatusAsync(owner, repo, pr.Head.Sha);
        }
        catch
        {
        ...
        try
        {
            reviews = await _githubClient.GetPullRequestReviewsAsync(owner, repo, pr.Number);
        }
        catch
        {
    ...
    public async Task<RebaseResult> RebaseAllOpenPRsAsync(string projectId)
    {
        ...
        var fetchResult = await _commandRunner.RunAsync("git", "fetch origin", project.LocalPath);
    ...
    private async Task<bool> RebaseBranchAsync(Project project, Feature feature, RebaseResult result)
    {
        ...
        // Perform rebase
        var rebaseResult = await _commandRunner.RunAsync(
            "git",
        ...
        if (!rebaseResult.Success)
        {
            ...
            await _commandRunner.RunAsync("git", "rebase --abort", workingDir);
        ...
        var pushResult = await _commandRunner.RunAsync(
            "git",
