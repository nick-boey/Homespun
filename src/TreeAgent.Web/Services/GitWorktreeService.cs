...
public class GitWorktreeService
{
    ...
    public GitWorktreeService(ICommandRunner commandRunner)
    {
    ...
    public async Task<string?> CreateWorktreeAsync(string repoPath, string branchName, bool createBranch = false, string? baseBranch = null)
    {
        ...
        if (createBranch)
        {
            ...
            var branchResult = await _commandRunner.RunAsync("git", $"branch \"{branchName}\" \"{baseRef}\"", repoPath);
        ...
        var result = await _commandRunner.RunAsync("git", args, repoPath);
    ...
    {
        var result = await _commandRunner.RunAsync("git", $"worktree remove \"{worktreePath}\" --force", repoPath);
        return result.Success;
    }

    public async Task<List<WorktreeInfo>> ListWorktreesAsync(string repoPath)
    {
        var result = await _commandRunner.RunAsync("git", "worktree list --porcelain", repoPath);
    ...
    {
        await _commandRunner.RunAsync("git", "worktree prune", repoPath);
    }
