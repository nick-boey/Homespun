...
### Requirement: Prompt catalogue CRUD

**Reason**: Replaced by filesystem skills. DB-backed `AgentPrompt` records, the two-tier catalogue (global + per-project overrides), template rendering engine (`{{placeholder}}` + `{{#if}}`), and all 13 HTTP endpoints under `/api/agent-prompts` are removed.

**Migration**: No migration needed. `AgentPrompt` table can be dropped. Prompts are now SKILL.md files under `.claude/skills/`.

### Requirement: Prompt defaults management

**Reason**: `DefaultPromptsInitializationService`, `default-prompts.json` seeding, ensure/restore/delete-all operations are removed. Skills are version-controlled files; there is no "seeding" concept.

**Migration**: No migration needed. Default skills are committed to the repository.

### Requirement: Prompt Category and SessionType routing

**Reason**: `Category` (Standard/IssueAgent) and `SessionType` enum routing are removed. The Task Agent tab dispatches any skill or no skill; the Issues Agent tab is skill-less; the OpenSpec tab hard-codes its 8 skills.

**Migration**: No migration needed. Tab-based routing replaces enum-based routing.
