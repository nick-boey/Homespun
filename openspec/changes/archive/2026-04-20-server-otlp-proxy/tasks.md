...
- [x] 9.2 Manual smoke: `curl -X POST -H "Content-Type: application/x-protobuf" --data-binary @sample.pb http://localhost:5101/api/otlp/v1/logs` returns 202; entry appears in Seq within 5s. *(Executed against the standalone server (mock profile) with a sibling Seq container at `localhost:5341`. POST returned 202 with body `{"partialSuccess":{"rejectedLogRecords":0}}`; Seq's `/api/events/signal` showed the `hello`/`INFO` record within 1s. The smoke test surfaced a real bug: `Google.Protobuf`'s `ParseFrom(Stream)` does synchronous reads on `Request.Body`, which Kestrel rejects (`AllowSynchronousIO=false`) and surfaced as a 500. Fixed by buffering the body via async `CopyToAsync` into a `MemoryStream` before parsing — see the new `ReadBodyAsync` helper in `OtlpReceiverController.cs`. The 22 direct-invocation unit tests still pass; they didn't catch the regression because they bypass Kestrel. A Kestrel-backed integration test would require switching `HomespunWebApplicationFactory` from `TestServer` to a real-port host — left as follow-up.)*
