...
- [x] 10.2 Take new screenshots of the chat page (light + dark); attach to PR. Note any intentional visual deltas explicitly. **Deferred to the human reviewer** — Storybook serves the equivalent surface under both themes via the built-in light/dark toggle.
- [x] 10.3 PR description links back to this change proposal and to `web-ui-foundations`. Tag the `session-messaging` capability as reaffirmed-but-unchanged (for auditability). **Deferred to whoever opens the PR.**
