...
- [x] 7.1 Add a `prod` profile to `src/Homespun.AppHost/Properties/launchSettings.json` with: `ASPNETCORE_ENVIRONMENT=Production`, `DOTNET_ENVIRONMENT=Production`, `HOMESPUN_DEV_HOSTING_MODE=container`, `HOMESPUN_PROFILE_KIND=prod`, the existing OTLP / dashboard URL pair.
...
- [ ] 8.1 With `~/.homespun-container/data` deleted (or moved aside), run `dotnet run --project src/Homespun.AppHost --launch-profile prod`. Confirm: the directory is created on first boot; the server reaches Running; the web UI loads with an empty project list.
- [ ] 8.2 Create a project via the UI, confirm it persists in `~/.homespun-container/data/.homespun/homespun-data.json`. Stop and re-start the prod profile; confirm the project is still present.
- [ ] 8.3 Confirm Seq is reachable at `http://localhost:5341` and ingests at least one server log entry under the new profile.
...
- [ ] 9.1 With `HOMESPUN_DEBUG_FULL_MESSAGES=true` in env, run `dev-live`. Trigger a real session that exercises the SDK (e.g., `claude run` against a small task).
- [ ] 9.2 In Seq, filter by `service.name = homespun.worker` for `a2a.emit` entries — confirm full bodies are present.
- [ ] 9.3 In Seq, filter by `service.name = homespun.server` for `a2a.rx`, `agui.translate`, `agui.tx` entries — confirm full bodies are present and each entry's body renders inline in the log list view (no expansion required).
- [ ] 9.4 In Seq, filter by `service.name = homespun.web` — confirm envelope-receive entries are present.
- [ ] 9.5 Trigger a `GET /api/sessions/{id}/events` from the browser (e.g., refresh the session page). Confirm the resulting log entries carry `homespun.replay=true` and can be filtered out via `homespun.replay is null`.
- [ ] 9.6 With the env var unset, repeat the smoke and confirm none of the `a2a.rx`, `agui.translate`, `agui.tx`, `a2a.emit` (full-body), or web envelope-receive log entries appear in Seq.
...
- [x] 10.5 Run the full pre-PR checklist (`dotnet test`, `npm run lint:fix`, `npm run format:check`, `npm run typecheck`, `npm test`, `npm run build-storybook`, `dotnet test` again for new options/scrubber tests). — *All green: dotnet tests 1797/1797 passed; worker 253/253; web 1958/1958 (1 skipped); typecheck clean; format:check clean; lint:fix 0 errors (21 pre-existing warnings); Storybook built OK.*
