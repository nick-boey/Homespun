...
**Reason:** Layout moves to the frontend (TypeScript port of `Fleece.Core.GraphLayoutService` + `IssueLayoutService`). The server no longer runs layout for the JSON response path; `IIssueLayoutService` is no longer injected into `ProjectFleeceService` and the corresponding DI registration is dropped. Filtering rules change from "active-status union additional ids" to "active-status plus ancestors-of-active plus explicitly-requested ids and their ancestors" — see the new `Visible issue set endpoint` requirement below.

**Migration:** the new `GET /api/projects/{projectId}/issues` endpoint replaces the layout-producing `GET /api/graph/{projectId}/taskgraph/data`. Callers (only the web client) consume the new endpoint and run layout via the TS port.

### Requirement: TaskGraphResponse exposes semantic edges and dimension counts

**Reason:** `TaskGraphResponse`, `TaskGraphNodeResponse`, `TaskGraphEdgeResponse`, and the laid-out wire shape are deleted. The visible-issue-set endpoint returns issues only — no positional data (no `Lane`, `Row`, `TotalLanes`, `TotalRows`, `Edges[]`, `Nodes[]`); positions and edges are derived on the client by the TS layout port.

**Migration:** the web client builds `PositionedNode<Issue>` and `Edge<Issue>` arrays locally via `layoutForTree` / `layoutForNext`. Decoration data (`AgentStatuses`, `LinkedPrs`, `OpenSpecStates`, `MergedPrs`, `MainOrphanChanges`) move to **independent endpoints** — see the new "Visible issue set endpoint", "Linked PRs endpoint", "Agent statuses endpoint" requirements (and the OpenSpec capability's "OpenSpec states endpoint" + "Orphan changes endpoint" requirements). The merged-PR list reuses the existing `GET /api/projects/{projectId}/pull-requests/merged` endpoint.
...
- Simple tree, deep tree, multi-parent diamond, series chain, parallel children, mixed series/parallel siblings, cycle (failure case), empty input, single node, large input (≥200 nodes from a real project export), `LayoutForNext` matched-leaves scenario, `LayoutForNext` large input.
