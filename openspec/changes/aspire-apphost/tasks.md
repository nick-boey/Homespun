...
- [ ] 3.4 Verify `dotnet run --project src/Homespun.AppHost --launch-profile dev-windows` boots with a pre-run worker container reachable at the injected `WorkerUrl`; smoke-test on Windows (or WSL) that SingleContainer mode routes sessions to it.  *(Runtime verification — covered by 8.3 smoke test.)*
...
- [ ] 8.4 Manual smoke test: `scripts/run-komodo.sh` (or a dry-run equivalent) still parses `.env` correctly and starts without touching user-secrets.  *(Deferred — prod-side runtime check; requires Komodo install on a VM.)*
