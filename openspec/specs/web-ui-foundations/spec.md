...
Keeps the `src/Homespun.Web` UI foundation aligned with Tailwind v4 idioms, provides a Storybook 10 harness for component-level verification against the real theme, and documents which `components/ui/*` files are shadcn-native, intentionally divergent, or short-lived chat-owned primitives.
