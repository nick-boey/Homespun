...
- **Fleece**: Integration with Fleece issue tracking - JSONL-based storage in `.fleece/` directory
  - **Version Sync Required**: When updating the `Fleece.Core` NuGet package version in `Homespun.Server.csproj` and `Homespun.Shared.csproj`, you must also update the `Fleece.Cli` version in `Dockerfile.base` to match. Current version: **3.0.0**.
